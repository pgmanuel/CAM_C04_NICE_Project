...
def build_config() -> Config:
    ...
    project_paths = build_project_paths(base_dir)
    snomed_path = Path(
        os.environ.get(
            "INTEGRATED_AGENT_SNOMED_PATH",
            str(project_paths.snomed_path),
        )
    )
    chroma_dir = Path(
        os.environ.get(
            "INTEGRATED_AGENT_CHROMA_DIR",
            str(project_paths.chroma_persist_dir),
        )
    )
    return Config(
        base_dir=base_dir,
        ...
        audit_dir=project_paths.audit_dir,
        regression_cases_path=project_paths.regression_cases_path,
